...
| Gitee mirror (raw, anonymous) | https://gitee.com/tomhands/vibecoding-manager/raw/main/releases/0.1.0/vibecoding-manager-0.1.0.zip |
| Gitee SHA-256 (raw, anonymous) | https://gitee.com/tomhands/vibecoding-manager/raw/main/releases/0.1.0/vibecoding-manager-0.1.0.zip.sha256 |
...
from Gitee's anti-bot layer while the Gitee API still serves the original files. The raw mirror
below is a repository file under `releases/0.1.0/`, intended for non-logged-in users.

Domestic fallback order: Gitee raw -> `gh-proxy.com` -> `ghfast.top`. Proxy services are
third-party and may change; always verify the downloaded zip against the published SHA-256.
