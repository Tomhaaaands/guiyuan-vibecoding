...
zip URL (see skills/vibe-coding-install/SKILL.md). This tool produces that zip: three
self-contained skills at the zip root, so unzipping into an agent's global skills root
installs the kit (no wrapper dir).

Usage:
  python tools/build_dist.py            # build dist/vibecoding-manager-<version>.zip
...
def build(out_dir: Path, verify: bool) -> Path:
    ver = version()
    zip_path = out_dir / f"vibecoding-manager-{ver}.zip"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build the distributable VibeCoding_Manager skill zip")
