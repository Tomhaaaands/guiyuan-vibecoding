...
| --- | --- | --- | --- | --- |
| R8 | 08-30 | release | v1.0.0 one-click install: template gains `review-checklist.md` + `roadmap.md` (both copies); check_drift adds template/asset sync gate (templates <-> assets, close-loop skill <-> asset copy); new `tools/one_click_install.py` + `install.bat`/`install.sh` wrappers (Python ≥3.11 check → install skills → doctor → optional `--target` scaffold); VERSION bumped 1.0.0; verified doctor + check_drift green and temp-project scaffold contains the new docs | [r8](docs/04-workflow/archive/2026-08-30-r8.md) |
